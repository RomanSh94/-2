...
def test_module_import_allowlist_is_exact():
    ...
    EvidenceKind, EvidenceRef, Intent, InteractionRequest, InteractionSignal,
    as_enum, validate_evidence_against_source} -- an exact allowlist, not a
    ...
    passing because it wasn't on a blacklist. Updated for Slice 2A-5: the
    module now legitimately needs EvidenceKind and EvidenceRef to
    canonicalize an external analyzer's untrusted EvidenceKind proposal
    into an EvidenceItem, added here in the same mutation -- reused
    unchanged from Stage 1, never duplicated."""
    ...
    expected_imports = [
        ...
        ("enum", ("Enum",)),
        ("therapeutic_domain", (
            "EvidenceItem", "EvidenceKind", "EvidenceRef", "Intent", "InteractionRequest",
            "InteractionSignal", "as_enum", "validate_evidence_against_source")),
    ]
