...
X20 развивается в сторону утверждённой владельцем профессиональной психологической
системы, определённой в каноническом источнике истины
[`X20_PROJECT_CONSTITUTION.md`](X20_PROJECT_CONSTITUTION.md). Целевое направление —
не универсальный чат-бот эмоциональной поддержки, а система, способная выполнять
как можно больше работы сильного психолога в безопасных и технически
поддерживаемых пределах.

Положения Конституции описывают целевое (TARGET) состояние продукта и не являются
доказательством того, что соответствующие возможности уже реализованы в коде —
см. раздел «Current implementation» ниже и сам код/тесты.

## 📋 Current implementation (CURRENT)

Текущая production-версия может реализовывать лишь часть целевого направления;
точный текущий функционал нужно проверять по коду и тестам, а не предполагать
из описания цели выше.
...
**Текущие границы безопасности (сохраняются и как целевые):**
- ❌ НЕ заявляет о статусе лицензированного психолога/врача, которого у бота нет
  (ни сейчас, ни как целевое состояние)
- ❌ НЕ ставит медицинские/психиатрические диагнозы
- ❌ НЕ управляет медикаментозным лечением
- ❌ НЕ заменяет живое общение или профессиональную помощь; при признаках
  экстренной ситуации — эскалация к кризисной/медицинской помощи
